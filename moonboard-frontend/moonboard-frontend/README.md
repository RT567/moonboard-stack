...
* [`src/moonboard_frontend/`](src/moonboard_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/moonboard_frontend/core.cljs): contains the SPA entry point, `init`
* [`test/moonboard_frontend/`](test/moonboard_frontend/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `moonboard-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/moonboard_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns moonboard-frontend.example
  (:require [moonboard-frontend.config :as config])
